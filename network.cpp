...
void Network::loadDB(string filename){
    ...
    while (getline(file, fname)) {
        ...
        std::string lname, bdate, emailLn, phoneLn,
                    collegeLn, majorLn, zipLn, divider;
        vector <pair<string,string>> temp;
        getline(file, lname);
        getline(file, bdate);
        getline(file, emailLn);                       // order: email first
...
{
    if (!head) { std::cout << "Database is empty.\n"; return; }

    /* ---- build an initial pool of every person ---- */
    std::vector<Person*> pool;
    for (Person* p = head; p; p = p->next) pool.push_back(p);
    ...
    while (true)
    {
        ...
        if (pool.empty()) { std::cout << "No records fit.\n"; return; }
        if (pool.size() == 1) {
            std::cout << "Unique record found:\n";
            pool.front()->print_person();
            return;
        }
        else if (pool.size() <= 8){
        	for (Person* p : pool) {
       			p->print_person();            // existing pretty‑printer
        		std::cout << "--------------------\n";
        	}
    	}

        
        std::cout << "Filter by field\n"
        ...
        std::cout << "Value (substring, case‑insensitive): ";
        std::string value;
        std::getline(std::cin >> std::ws, value);
        std::string vLower = lowerCopy(value);

        std::vector<Person*> next;
        for (Person* p : pool) {
            std::string candidate = lowerCopy(p->getField(field));
            if (!candidate.empty() &&
                candidate.find(vLower) != std::string::npos)
                next.push_back(p);
        }
        pool.swap(next);
    }
