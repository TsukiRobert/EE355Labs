...
bool Person::matches(const std::string& q) const
{
    std::string key = lower(q);

    auto has = [&](const std::string& field) {
        return lower(field).find(key) != std::string::npos;
    };

    // basic identity
    if (has(f_name) || has(l_name) ||
        has(f_name + " " + l_name) ||
        has(codeName(f_name, l_name)))           return true;

    // fixed attributes
    if (has(college)       || has(major)     || has(zip_code) ||
        has(area_code)     || has(email_service))             return true;

    // date of birth in several common forms
    std::string dob = birthdate->get_date();      
    if (has(dob))                                 return true;

    // phone & email contacts
    if (has(phone->get_contact())  || has(email->get_contact()))
        return true;
