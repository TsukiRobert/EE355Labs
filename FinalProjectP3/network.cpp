...
{
    if (head == nullptr) { std::cout << "Database is empty.\n"; return; }

    std::cout << "Enter ANY fragment to search: ";
    std::string query;
    std::getline(std::cin >> std::ws, query);

    bool found = false;
    for (Person* p = head; p; p = p->next) {      // your existing list walk
        if (p->matches(query)) {
            p->print_person();                    // or p->printSummary()
            std::cout << "--------------------\n";
            found = true;
        }
    }
    if (!found) std::cout << "No match for \"" << query << "\"\n";
}
...
void Network::showMenu(){
    ...
    while(1){
        ...
        cout << "6. Connect \n";
        cout << "7. Wise search (by any field) \n";
        cout << "\nSelect an option ... ";
